...
const APPS = config.get('APPS').map(x => {
  ...
  return _extends({}, x, { actions: appDefaults.actions // @TODO: allow the user to specify a subset of actions
  });
});
...
app.post('/v1/jobs', [
// validation
check('appKey').exists().custom((value, { req }) => appExists(APPS, value)).withMessage(`App does't exist`), check('payload').exists(), check('action').exists().custom((value, { req }) => {
